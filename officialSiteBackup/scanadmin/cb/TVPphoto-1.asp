...
rs4.close
set rs4=nothing
response.write"<script language=javascript>window.location.href='TVPphoto-1.asp?id="&request("id")&"&page="&request("page")&"';</script>"
response.end
end if
...
Recordset3.Source = "SELECT * FROM c ORDER BY ID DESC"
...
Recordset4.Source = "SELECT * FROM cc ORDER BY ClassID ASC"
...
<table width="900" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    ...
    <td width="900" height="33" valign="top"><table width="95%" border="0" cellpadding="0" cellspacing="0" background="../images2/products_04.jpg">
        <!--DWLayoutTable-->
        <tr bgcolor="f2f2f2"> 
          <td width="333" height="33" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
              <!--DWLayoutTable-->
              <tr> 
                <td width="433" height="33"><table width="88%" border="0" align="center">
                    <tr> 
                      <td>您的位置是：產品管理</td>
                    </tr>
                  </table></td>
          ...
          <td width="467"><table width="100%" border="0" align="right">
              <tr> 
                <td width="53%" height="33"><form name="form2" action="TVPphoto-11.asp">
...
%>
                                        <option value="<%=(Recordset4.Fields.Item("ClassID").Value)%>" <%If (Not isNull((Recordset4.Fields.Item("ClassName").Value))) Then If (CStr(Recordset4.Fields.Item("ClassID").Value) = CStr((Recordset4.Fields.Item("ClassName").Value))) Then Response.Write("SELECTED") : Response.Write("")%> ><%=(Recordset4.Fields.Item("ClassName").Value)%></option>
                                        <%
  Recordset4.MoveNext()
Wend
...
%>
  ...
  <tr> 
    <td height="570" valign="top" align="center"><table width="100%" border="0" cellpadding="0" cellspacing="0">
        <!--DWLayoutTable-->
        <tr> 
          <td width="850" height="570"><table width="98%" height="574" border="0" align="center">
              <tr> 
...
%>
                          <td> <table width="95%" border="0">
                              <tr> 
                                <td height="100" class="t2"><div align="center"><a href="TVPproductshow.asp?ID=<%=(Recordset3.Fields.Item("ID").Value)%>" ><img src="images\photo\<%=(Recordset3.Fields.Item("ProImg").Value)%>"  onload='DrawImage(this,100,100);' border="0"></a></div></td>
                              ...
                              <tr> 
                                <td width="4%" height="26" class="t2"><div align="center"> 
                                    <table width="85%" border="0">
                                      <tr> 
                                        <td align="center" class="t1"><table><tr ><td><div align="center"><%=(Recordset3.Fields.Item("ProName").Value)%></div></td><td>           
                                    ...
                                    <table width="100%" border="0">
                                      <tr> 
                                        <td bgcolor="e7e7e7"> <div align="left"><A HREF="photo-3.asp?<%= MM_keepNone & MM_joinChar(MM_keepNone) & "ID=" & Recordset3.Fields.Item("ID").Value %>"></A></div>
                                          <div align="center" class="t2">[<a href="works_id_manage.asp?<%= MM_keepNone & MM_joinChar(MM_keepNone) & "ID=" & Recordset3.Fields.Item("ID").Value %>"><font color="#666666">修改</font></a>][<a href="TVPphoto-5.asp?<%= MM_keepNone & MM_joinChar(MM_keepNone) & "ID=" & Recordset3.Fields.Item("ID").Value %>"><font color="#666666">刪除</font></a>] <!--%if Recordset3.Fields.Item("New").Value=0 then%>
          <a href="?action=looked&range=<'%=Recordset3.Fields.Item("ID").Value%>&page=<'%=page%>" title="设为封面">[封面]</a>
          <%'else%>
          <a href="?action=nolooked&range=<'%=Recordset3.Fields.Item("ID").Value%>&page=<'%=page%>" title="设为普通内页">[内页]</a>
          <%'end if%>--></div></td>
                                      </tr>
                                    </table>
                                  </div>
 ...
 %>
                      ...
                      <table width="100%" border="0" align="center" class="t2">
                        <tr bgcolor="f7f7f7"> 
                          <td width="51%" align="center">記錄<%=(Recordset3_first)%> 
                            到<%=(Recordset3_last)%> （總共<%=(Recordset3_total)%> 
                          個記錄）</td>
                          <td width="49%" align="center"> 
                            <% If MM_offset <> 0 Then %>
                            <a href="<%=MM_moveFirst%>"><font color="#666666">第一頁</font></a> 
                            <% End If ' end MM_offset <> 0 %>
                            <% If MM_offset <> 0 Then %>
                            <a href="<%=MM_movePrev%>"><font color="#666666">前一頁</font></a> 
                            <% End If ' end MM_offset <> 0 %>
                            <% If Not MM_atTotal Then %>
                            <a href="<%=MM_moveNext%>"><font color="#666666">下一頁</font></a> 
                            <% End If ' end Not MM_atTotal %>
                            <% If Not MM_atTotal Then %>
                            <a href="<%=MM_moveLast%>"><font color="#666666">最後一頁</font></a> 
